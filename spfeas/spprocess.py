...
def _section_reader(section_pair):

    i_sect = section_pair[0]
    j_sect = section_pair[1]

    n_rows = raster_tools.n_rows_cols(i_sect, this_parameter_object.sect_row_size, this_image_info.rows)
    n_cols = raster_tools.n_rows_cols(j_sect, this_parameter_object.sect_col_size, this_image_info.cols)

    # Open the image array.
    # TODO: add other indices
    if this_parameter_object.trigger in ['ndvi', 'evi2']:

        sect_in = this_image_info.read(bands2open=[this_parameter_object.band_red,
                                                   this_parameter_object.band_nir],
                                       i=i_sect,
                                       j=j_sect,
                                       rows=n_rows,
                                       cols=n_cols,
        ...
        sect_in = vie.compute(this_parameter_object.trigger.upper(), out_type=2)

        this_parameter_object.min = 0
        this_parameter_object.max = 255

    elif this_parameter_object.trigger == 'dmp':
    ...
    elif this_parameter_object.trigger == 'saliency':

        sect_in = spsplit.saliency(this_image_info,
                                   this_parameter_object,
                                   i_sect,
                                   j_sect,
                                   n_rows,
                                   n_cols)

    elif this_parameter_object.trigger == 'grad':
        ...
        this_parameter_object.update_info(min=0, max=255)

    elif this_parameter_object.use_rgb and this_parameter_object.trigger not in ['grad', 'ndvi', 'evi2', 'dmp', 'saliency']:
        ...
        sect_in = this_image_info.read(bands2open=this_parameter_object.band_position,
                                       i=i_sect,
                                       j=j_sect,
                                       rows=n_rows,
                                       cols=n_cols)

    # if this_parameter_object.trigger == 'orb':
    #     sect_in = spsplit.get_orb_keypoints(sect_in, this_parameter_object)
    #     this_parameter_object.update_info(min=0, max=255)

    # pad array here
    # (top, bottom), (left, right)
    sect_in = sputilities.pad_array(this_parameter_object, sect_in, n_rows, n_cols)

    if this_parameter_object.trigger == 'dmp':
    ...
    if sect_in.max() > 0:
        ...
        out_section_array = spreshape.reshape_feas(this_parameter_object.trigger,
                                                   tk,
                                                   oR,
                                                   oC,
                                                   l_rows,
    ...
    print out_section_array
    sys.exit()

    # if isinstance(out_section_array, np.ndarray)
    ...
    if parameter_object.stack_only:
        ...
        for trigger in parameter_object.triggers:
            ...
            for band_position in parameter_object.band_positions:
                ...
                Parallel(n_jobs=1)(delayed(_section_reader)(parameter_object.section_idx_pairs[idx_pair])
                                   for idx_pair in range(0, parameter_object.n_sects))
